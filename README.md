...
> 📚 **التوثيق الشامل في [`docs/`](./docs/README.md)** — ابدأ منه. المراجع الجذرية: `GUIDELINES.md` (دستور المشروع) · `BLUEPRINT.md` (المرجع الوظيفي) · `ROADMAP.md` (الخطة) · `DESIGN.md` (رموز التصميم) · `CHANGELOG.md` (سجل التغييرات) · `design-references/` (التصميم البصري المعتمد).
...
حارس موحّد على كل endpoint معلَّم بـ `@Authorize` يفحص **فحصين** (GUIDELINES.md §3): هل الموديول مفعّل في باقة المستأجر؟ + هل لدور المستخدم صلاحية الفعل؟
