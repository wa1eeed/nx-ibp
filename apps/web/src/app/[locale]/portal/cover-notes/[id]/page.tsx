...
export default function PortalCoverNotePage({ params }: { params: { id: string } }) {
  ...
  const load = useCallback(async () => {
    try {
      const [d, br] = await Promise.all([cpapi<CoverDoc>(`/portal/cover-notes/${params.id}/document`), cpapi<typeof b>("/portal/branding")]);
      setDoc(d); setB(br);
  ...
  return (
    <div className="mx-auto max-w-[820px] p-4 print:p-0">
      <div className="mb-4 flex items-center justify-between print:hidden">
        <Link href="/portal/dashboard" className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-primary hover:underline"><ArrowLeft size={15} className="rtl:rotate-180" /> {t("back")}</Link>
