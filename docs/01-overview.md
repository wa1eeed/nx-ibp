...
**الرؤية:** أن تكون المنصة المعيارية لأي وسيط تأمين سعودي، قابلة للتهيئة بالكامل دون تخصيص برمجي، ومتوافقة من اليوم الأول مع متطلبات **هيئة التأمين** و**نظام حماية البيانات الشخصية (PDPL)** وضوابط **الهيئة الوطنية للأمن السيبراني (NCA)**، ومجهّزة للتكامل مع المنظومة الحكومية الرقمية (نفاذ، يقين، واثق، العنوان الوطني، ZATCA).
...
### الباقات المرجعية
يعرّفها السوبر أدمن ويضبط أسعارها لكل مستخدم (نموذج `Plan` في المخطط: `code`, `priceMonthly`, `priceYearly`؛ `seatLimit` أصبح **nullable = بلا سقف** — التسعير لكل مستخدم فعلي).

| الباقة | الرمز (`Plan.code`) | الجمهور | السمة الأبرز |
|---|---|---|---|
| الأساسية | `basic` | وسطاء صغار | **دورة تشغيل الوساطة كاملة** (عملاء→مبيعات→اكتتاب→إنتاج→تجديدات→خدمة→مطالبات→مالية أساسية→تقارير+تحقّق). مقفلة: ZATCA وكل مزايا النمو |
| الاحترافية | `premium` | معظم شركات الوساطة (الباقة الطبيعية) | كل الأساسية + **ZATCA والمالية المتقدّمة وسلاسل الاعتماد وCRM والمنتجون والتحليلات والهيكل + MFA إلزامي** |
| المؤسسات | `enterprise` | مجموعات وكيانات كبيرة | كل الاحترافية + **API وWhite-Label وDLP والموارد البشرية وتدقيق متقدّم ودعم وSLA** — «تواصل مع المبيعات» |

> أمثلة من الـ seed (بيانات وهمية): مستأجر «وكالة الخليج» = `premium`؛ مستأجر «شركة الأمان» = `basic` (ZATCA والتحليلات مقفلة؛ المطالبات **مفعّلة** كموديول أساسي تتطلّبه هيئة التأمين). راجع [`README.md`](../README.md).

### محورا التحكم
1. **المستخدمون (Seats):** **رخصة مقاعد مسبقة الدفع** (`Subscription.seatsLicensed`) — التسعير لكل مستخدم؛ الرخصة = المقاعد المختارة عند التسجيل، ولا يتجاوز النشطون الرخصة (إضافة تتخطّاها ⇒ 402). لإضافة المزيد ⇒ **شراء مقاعد** (احتساب تناسبي للمدّة المتبقّية) يرفع الرخصة عند الدفع.
2. **الموديولز/الميزات:** ما هو غير مفعّل في الباقة يُفعَّل بالترقية/شراء add-on.
...
| 9 | التكاملات التنظيمية (ZATCA، يقين، نفاذ، واثق، العنوان الوطني…) + النشر داخل المملكة | ✅ منجزة ومُختبرة |
| **4ب+** | جاهزية ZATCA Fatoora المرحلة 2 (تهيئة/فوترة/توجيه معزولة) | ✅ منجزة ومُختبرة |

**الحالة:** المنصّة مكتملة وظيفيًا + **كل النواقص البرمجية منجزة** (تخزين S3/تسجيل ذاتي/فوترة اشتراكات/أقسام/حصص تخزين/إثراء حقول/ضريبة بالفرع E1/الإشعارات + مركز in-app/سلاسل اعتماد E2/تحصين أمني + MFA/سجل تدقيق ثابت/الاحتفاظ + DLP + **خارطة تجاوز أويسس 8/8** (التحصيل/الإلغاء/اتجاه الفاتورة/تسوية المؤمِّن/المبيعات/**الوسطاء الفرعيون**/فصل الاكتتاب/المرجعية IA) + **مكتبة قوالب النماذج** + **البريد BYO/الهوية البصرية/الأهداف/التملّك/إدارة شركات التأمين/سجل التدقيق كصفحة/طلبات التواصل** + **نظرة المالك المالية/تطوير الخدمة/إثراء CRM/الملاحق الاحترافية/امتثال فاتورة ZATCA** + **المنتجات + المستودع المركزي للمستندات + صلاحيات على مستوى المنتج + نموذج التحصيل القابل للتهيئة**) + **بدء إكمال القسم المالي بعد التدقيق: القيود اليدوية/المصروفات + عمولات الموظفين + خطة تقسيط الأقساط + الميزانية العمومية + دفتر الأستاذ + إقرار ض.ق.م + أعمار الذمم + توفير دخول بوّابة العميل + بوّابة الدفع الإلكتروني (Tap/Moyasar) + سجلّ الشكاوى + كشف المؤمِّن الدوري + بيان التدفّق النقدي (القوائم المالية الثلاث مكتملة) + مكافحة غسل الأموال (AML: تقييم مخاطر + فرز عقوبات/PEP + بلاغات اشتباه) + عرض العروض للعميل وقبوله (بوّابة) + مذكرة التغطية المؤقتة**) — **e2e 386/386**. السرد الحاكم لكل ما بعد الاكتمال في [`docs/34`](./34-post-completion-features.md)، وحالة الاستئناف في [`docs/00`](./00-project-status.md).
