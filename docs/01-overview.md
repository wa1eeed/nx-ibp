...
هذا المستند هو نقطة الدخول التوثيقية للمشروع. يلخّص الرؤية ونموذج العمل واللوحات والعائلات التأمينية والمبادئ التأسيسية وحالة المراحل. للتفصيل الوظيفي الكامل راجع [`BLUEPRINT.md`](../BLUEPRINT.md)، ولخطة التنفيذ [`ROADMAP.md`](../ROADMAP.md)، ولدستور المشروع [`CLAUDE.md`](../CLAUDE.md).
...
> ملاحظة تنظيمية: انتقل الإشراف على قطاع التأمين إلى **هيئة التأمين (Insurance Authority)** المنشأة عام 2023، وتولّت مهامًا كانت لدى مؤسسة النقد سابقاً؛ لذا تُوجَّه التقارير الرقابية إلى هيئة التأمين، والفوترة الإلكترونية إلى ZATCA. التفاصيل في [`docs/OASIS-IBS-REQUIREMENTS.md`](./OASIS-IBS-REQUIREMENTS.md).
...
| **4أ** | الاكتتاب الفني وعروض الأسعار (RFQ): `Slip` + `Quotation` + المقارنة الآلية + الإسناد | ✅ منجزة ومُختبرة (e2e 36/36) |
| **4ب** | الإصدار والهندسة المالية (Finance Core): الاعتماد المزدوج + COA + السندات + الإشعارات | 🔜 قادمة |
| 5 | وحدة المستندات (Presigned URLs + ضغط + عزل بالمسار) | 🔜 |
| 6 | بقية الموديولز التشغيلية (خدمة، مطالبات، تجديدات) | 🔜 |
| 7 | التحقق الحكومي (يقين/واثق/نفاذ + Sandbox) | 🔜 |
| 8 | اللوحات (سوبر أدمن، بوابة العميل) + التقارير | 🔜 |
| 9 | التكاملات التنظيمية (ZATCA، نجم…) + النشر داخل المملكة | 🔜 |

تقدّم التنفيذ المرحلي وتفاصيله موثّقة في `docs/18` (سجل المراحل) — يُنشأ عند توثيق المرحلة 4ب.
...
- [`docs/OASIS-IBS-REQUIREMENTS.md`](./OASIS-IBS-REQUIREMENTS.md) — مصفوفة الأمان ودورة العمل المتتبَّعة.
- [`CLAUDE.md`](../CLAUDE.md) — دستور المشروع وقواعده غير القابلة للتفاوض.
