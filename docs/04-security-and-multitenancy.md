...
- [2. عزل المستأجرين بثلاث طبقات](#2-عزل-المستأجرين-بثلاث-طبقات)
  ...
  - [2.4 نطاق المنصّة (Platform Scope)](#24-نطاق-المنصة-platform-scope--تخطٍّ-مقصود-ومحصور)
  - [2.5 نطاق العميل (Client Scope)](#25-نطاق-العميل-client-scope--عزل-مزدوج)
...
### 2.4 نطاق المنصّة (Platform Scope) — تخطٍّ مقصود ومحصور

مالك المنصّة (السوبر أدمن) يحتاج رؤية **عابرة لكل المستأجرين** (إدارة الباقات والاشتراكات والاستخدام). يتحقّق هذا دون ثقب في العزل، عبر مسار تفويض مستقلّ:

1. **كيان منفصل:** `PlatformAdmin` (بلا `tenantId`)، توكنه يحمل `scope: "platform"` بلا `tenantId`.
2. **سياق بلا مستأجر:** `TenantContextMiddleware` يرى `scope === "platform"` ⇒ يضبط `isSuperAdmin = true` و`store.tenantId = undefined`.
3. **تخطّي الفلترة:** Prisma `$use` يرى `tenantId` غير معرّف ⇒ لا يحقن `where` ⇒ الاستعلام يعبر كل المستأجرين (نفس آلية الإقلاع/الدخول، لكن مقصودة هنا).
4. **بوّابة مستقلّة:** الوصول لهذا النطاق محصور بـ `PlatformGuard` على مسارات `/platform/*` فقط.

**العزل ثنائي الاتجاه ومُثبَت بالاختبار:** مستخدم المستأجر يُرفض من `/platform` (403)، ومدير المنصّة يُرفض من مسارات المستأجر (403) لأنه بلا `roleId`/`tenantId` فيسقط فحص RBAC والـ entitlement. أي «التخطّي» قرارُ تفويضٍ صريح لا إخفاءُ مسار — التفصيل الكامل في [24 — لوحة السوبر أدمن](./24-platform-super-admin.md).

### 2.5 نطاق العميل (Client Scope) — عزل مزدوج

بوّابة العميل ([25](./25-client-portal.md)) عكس نطاق المنصّة: بدل تخطّي العزل، **تحافظ عليه وتُشدّده**. توكن العميل يحمل `scope:client` + `tenantId` + `clientId`:

1. `tenantId` يدخل ALS ⇒ Prisma middleware يفلتر كل استعلام بمستأجر العميل (الطبقة القائمة).
2. فوقها، `PortalService` يضيف `where: { clientId }` صراحةً ⇒ العميل لا يرى إلا بياناته هو، لا بيانات عملاء آخرين في نفس المستأجر.
3. `PortalGuard` يشترط `scope === "client"` و`clientId` ⇒ يمنع الموظف/السوبر أدمن.

مُثبَت بالاختبار: عميل مستأجر آخر لا يرى وثائق العميل، موظف المستأجر يُرفض من `/portal` (403)، والعميل يُرفض من مسارات المستأجر (403).
