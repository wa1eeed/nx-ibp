...
| **الحدود** (مستخدمون · تخزين) | **مقاعد مرخّصة مسبقة الدفع** (تسعير لكل مستخدم؛ الرخصة تُرفَع بالشراء) · 1GB | كما هي · 10GB | كما هي · 100GB |
...
### 7ج. فرض RBAC في الواجهة + مصفوفة صلاحيات الموظف

الحماية الأصل على **الخادم** (`@Authorize` يردّ 403). فوقها طبقة **تجربة** في الواجهة تُخفي ما لا يملكه المستخدم بدل أن يضغطه فيُرفَض:

- **`hooks/usePermissions`**: يقرأ مصفوفة `/auth/me` (`{access, create, edit, delete, revert}` لكل موديول) **مرّة** ويخزّنها على مستوى الموديول (`clearPermsCache` عند الخروج) — يوفّر `can(module, action)` و`ready`. مكوّن **`<Can module action fallback>`** إعلاني يُظهر أطفاله فقط إن مَلَك المستخدم الفعل.
- **التطبيق (كل الصفحات التشغيلية):** أزرار التنفيذ محكومة بصلاحية الفعل على كل صفحة — العملاء/الطلبات/الوثائق/المطالبات/الخدمة/CRM/المنتجات/المؤمِّنون/العمولات/الأقساط/الوسطاء/الشكاوى/AML + المالية والتجديدات (#109). فمثلًا **مدير الالتزام (المالية قراءة فقط)** لا يرى أزرار التسوية/التقسيط/المرتجع/القيود، بينما يرى أزرار الالتزام (اعتماد العميل/الشكاوى/AML). ملاحظة: الواجهة تفحص عمود المصفوفة **`edit`** (يقابل فعل الخادم `update`).

**مصفوفة صلاحيات الموظف (داخل شاشته):** تفاصيل الموظف (`/tenant/settings/staff/:id`) فيها تبويب **«الصلاحيات»** (افتراضي) يعرض **جدولًا كاملًا**: صفّ لكل موديول من `RBAC_MODULES` × أعمدة (وصول/إضافة/تعديل/حذف/تراجع)، مشتقّ من **دور** الموظف — لأن `GET /staff/:id` صار يُرجع `user.role.permissions` (المصفوفة كاملة). الموديولز بلا وصول تظهر خافتة. رأس الجدول يُبرز اسم الدور (شارة مُعدّ/مخصّص) وزرّ **«تعديل صلاحيات الدور»** ⇐ محرّر الأدوار (§7ب) — فالتعديل يتمّ **على الدور** فيسري على كل موظفيه (المعيار العالمي: صلاحيات مبنية على الأدوار لا على المستخدم الفرد). هذا يجيب سؤال «ما صلاحيات هذا الموظف؟» بصريًّا داخل ملفّه.
