...
### `POST /slips/:id/present` — عرض العروض على العميل (§4.1)

| | |
|---|---|
| الحماية | `underwriting:update` + `module.production` |
| رمز النجاح | **`200`** |
| الجسم | `{ quotationIds: string[] }` (غير فارغة) |

يعرض عروضًا **منتقاة** على العميل في بوّابته (يبدأ قراره `pending`) — يختار الوسيط ما يُظهره. يتطلّب أن للطلب عميلاً (وإلا `422`)، ولا يُسمح على طلب أسعار مُسنَد/مغلق (`409`). إشعار عميل `proposal_ready`. **الأخطاء:** `400` بلا عروض · `404` · `409` · `422`.

**عروض البوّابة (نطاق العميل — §4.1):**

| الفعل | المسار | ملاحظة |
|---|---|---|
| `GET` | `/portal/proposals` | العروض المقدَّمة للعميل + حالة قراره + عدد الخيارات |
| `GET` | `/portal/proposals/:id` | خيارات العرض **بلا بيانات عمولة الوسيط** + قرار العميل |
...
| GET | `/search?q=` | مصادقة فقط | **البحث العام (⌘K)**: بحث موحّد عبر العميل/الوثيقة/المطالبة/الطلب/شركة التأمين بالاسم/الرقم (6 لكل نوع). **تصفية داخلية حسب صلاحية القراءة لكل نوع** (RBAC) + عزل بالمستأجر. استعلام <حرفين ⇒ نتائج فارغة |
...
| GET | `/finance/cash-flow?from&to` | finance:read | **بيان التدفّق النقدي** (مباشر) — حركة النقد (0101) مصنّفة تشغيلي/استثماري/تمويلي حسب الطرف المقابل · افتتاحي + صافي التغيّر = ختامي · `reconciles` · فترة اختيارية |
