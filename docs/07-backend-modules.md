...
| الوحدة | الغرض |
|---|---|
| `notifications` | نظام الإشعارات لكامل النظام (33 نوعًا، عملاء+موظفون، `notify`/`notifyStaff`/`notifyUser`) + مركز in-app + بوّابة seam (Taqnyat/Resend) |
...
> **الحالة الفعلية:** كل المراحل 0–9 + ZATCA P2 + مسارات ما بعد الاكتمال (E1–E5/الإشعارات/الأمن) + **خارطة تجاوز أويسس 8/8** + سجلّ الوسطاء الفرعيين + مكتبة قوالب النماذج **مبنية ومُختبَرة** + إكمال القسم المالي (القوائم المالية الثلاث) + عنقود الامتثال (شكاوى/كشف المؤمِّن/AML) (e2e 316/316). راجع [`docs/00`](./00-project-status.md) و[`docs/29`](./29-roadmap-next.md).
