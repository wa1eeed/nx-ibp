...
| **المجالية** | [`src/modules/`](../apps/api/src/modules) | منطق الأعمال: auth, rbac, catalog, clients, requests, underwriting, production, finance, documents, service, claims, renewals, verification, **platform**, **portal**, **reports**, **compliance**, **regulatory**, staff, health |
...
| `auth` + `tenant-context` (ALS) | 1 — المصادقة والعزل | ✅ مبنية | JWT + سياق المستأجر + Prisma guard |
...
| `clients` | 3 — العملاء | ✅ مبنية | كود تجاري + بوّابة الالتزام |
...
| `claims` | 6 — المطالبات | 🟡 بوّابة entitlement فقط | نقطة قراءة مصغّرة لإثبات الفحص المزدوج |
| المالية والإصدار (`Finance Core`) | 4ب | ⏳ المخطط مهيّأ، المنطق لاحقاً | الكيانات موجودة في [schema](../packages/db/prisma/schema.prisma) |
| المستندات / الخدمة / التحقق / التقارير / HR | 5–8 | ⏳ لاحقاً | راجع [`modules/README.md`](../apps/api/src/modules/README.md) |

> الحالة الفعلية تطابق ما هو مُسجَّل في [`src/modules/README.md`](../apps/api/src/modules/README.md) و[ROADMAP.md](../ROADMAP.md): المراحل 0→4أ مكتملة، 4ب التالية.
