...
### 3.8 المرحلة 4ب — الإصدار والمالية (مخطّط لاحقاً)

التسلسل المستهدف ([ROADMAP.md](../ROADMAP.md) §4ب) — الكيانات مهيّأة في المخطط، المنطق يُبنى في مرحلتها:

1. إصدار `Policy` من العرض المختار ⇒ الطلب `UNDER_REVIEW`.
2. **اعتماد فني** ⇒ يبقى `UNDER_REVIEW` ثم يُمرَّر `FINANCE_REVIEW`.
3. **اعتماد مالي** ⇒ `APPROVED` ثم `ISSUED`.
4. توليد آلي: قيد محاسبي (`Voucher` JRV)، إشعار مدين/دائن للعميل (`DebitNote`/`CreditNote`)، وفاتورة ضريبية لشركة التأمين (`Invoice`، تمهيد ZATCA).
5. فتح حساب تحليلي للعميل في شجرة الحسابات (`ChartOfAccount`).

كل خطوة تُسجَّل في `auditLog` (التحقّق: صفقة كاملة من الإنشاء حتى القيد).
