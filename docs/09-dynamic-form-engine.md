...
- [4. مفاتيح الكتل (BlockKey) وكيف يتكيّف النموذج لكل منتج](#4-مفاتيح-الكتل-blockkey-وكيف-يتكيف-النموذج-لكل-منتج)
  - [4.1 الاستيراد الجماعي من CSV](#41-الاستيراد-الجماعي-من-csv-bulk-import)
...
### 4.1 الاستيراد الجماعي من CSV (Bulk Import)

الإدخال صفًّا صفًّا مقبول لعقد بمركبة أو مركبتين، لكنه غير عملي **لأسطول** من 200 مركبة أو **مجموعة** طبية بألف تابع. لذلك كل كتلة متكررة تعرض — إلى جانب زرّ «إضافة صف» — زرّين: **«تنزيل قالب»** و**«استيراد CSV»** ([`DynamicForm.tsx`](../apps/web/src/components/forms/DynamicForm.tsx)). المنطق كله في وحدة نقية بلا اعتماديات: [`lib/block-csv.ts`](../apps/web/src/lib/block-csv.ts).

**المبدأ:** القالب والمُحلِّل مشتقّان **من نفس `BlockDef.fields`** التي يبني منها النموذج حقوله — فلا يوجد تعريف عمود منفصل يمكن أن يتخالف مع المخطط. إضافة حقل للكتلة يظهر تلقائياً عموداً في القالب وقاعدةَ تحقّق في الاستيراد.

| الدالة | الدور |
|---|---|
| `blockTemplateCsv(block, locale)` | يولّد قالب CSV: صف ترويسة بتسميات الحقول (`labelAr`/`labelEn` حسب اللغة، وتُلحَق `*` بالإلزامي) + صف مثال واحد. يُصدَّر بترميز **BOM** (`﻿`) ليفتح صحيحاً في Excel العربية بلا تشويه المحارف. |
| `parseCsv(text)` | مُحلِّل CSV نقي يدعم: الاقتباس المزدوج، الفواصل داخل الاقتباس، أسطر CRLF/LF، وتجريد BOM. لا مكتبة خارجية. |
| `parseBlockCsv(text, block, locale)` | يطابق كل عمود بترويسته إلى حقل (`labelAr` أو `labelEn` أو `key`)، يحوّل النوع (`number`/`currency`/`percent` ← رقم، الباقي نص)، ثم **يتحقّق من كل صفّ** بنفس قواعد الكتلة (مطلوب/خيارات `select`/هوية 10 أرقام/حدود). |

**تقرير النتيجة** بنية `{ rows, errors, skipped }`: الصفوف الصالحة تُضاف إلى حالة الكتلة، والأخطاء تُعرض بقائمة دقيقة **بالصف والعمود والسبب** (مثال: «الصف 4 — نوع المركبة: قيمة غير مسموحة ‹طيران›»). الصفوف الفارغة تُتخطّى بلا خطأ. المستخدم يصحّح ملفه ويعيد الرفع — لا إدخال يدوي.

**حدود الثقة:** الاستيراد راحةُ إدخال في الواجهة فقط؛ **التحقّق النهائي يبقى في الخادم** ([§6](#6-محرّك-التحقق-formvalidationservice)) — الحمولة المُجمَّعة من CSV تمرّ على `FormValidationService.validate` مثل أي حمولة، فلا يفتح الاستيراد ثغرة تجاوز للتحقّق.
