...
| [`platform.e2e-spec.ts`](../apps/api/test/platform.e2e-spec.ts) | 9 | دخول السوبر أدمن، **رؤية كل المستأجرين عابرةً للعزل**، رفض المستأجر من `/platform` ورفض المنصّة من مسارات المستأجر (403)، الاستخدام، ضبط entitlement، تعليق/تفعيل، **وتعطيل `feature.zatca` من الباقة ⇒ 403 مُفرَض على المستأجر ثم استعادته** |
...
**الإجمالي: 260 اختباراً (38 ملفاً).**
