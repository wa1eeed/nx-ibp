...
| الحساب | مدين | دائن |
|---|---|---|
| `0103` ذمم العملاء المدينة | الإجمالي T (القسط + ضريبته) | — |
| `0202` أمانات أقساط العملاء (Off‑Balance) | — | T − C − VATₒ |
| `0401` عمولات الوساطة (إيراد) | — | العمولة C |
| `0203` ضريبة القيمة المضافة المستحقة (Output VAT) | — | VATₒ = C × 15% |

حيث **VATₒ = ضريبة مخرجات الوسيط على العمولة**.

**ضريبتان منفصلتان (وفق المعيار السعودي):**
...
2. **ضريبة العمولة (15%)** = مخرجات **الوسيط** التي يورّدها لـ ZATCA؛ تُقيَّد كالتزام `0203` وتظهر على **الفاتورة الضريبية للمؤمِّن** (commission + 15%).
...
[`test/finance.e2e-spec.ts`](../apps/api/test/finance.e2e-spec.ts): الشلال الكامل، منع RBAC (المبيعات/المحاسب من الإنتاج، المكتتب من المالية)، **توازن القيد** (مدين = دائن)، قيم الفاتورة/الإشعار، انتقال الطلب إلى ISSUED، منع إعادة الاعتماد (409)، **المعالجة الضريبية** (ضريبة القسط 15% على إشعار العميل، ضريبة العمولة 15% على فاتورة المؤمِّن مُقيَّدة كـ Output VAT)، والعزل. **e2e 103/103**.
