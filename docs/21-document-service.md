...
- `ATTACHMENT`: قابل للضغط — صور المرفقات تُحوَّل تلقائيًا إلى **WebP** (جودة 80، أقصى بُعد 1200px) عبر `sharp` في المسار المحلي (`StorageService.maybeCompress`، المرحلة D2)، والحصّة تُطابَق بالحجم المضغوط. الرفع السحابي المباشر يحتاج worker خلفي (مؤجَّل).
