...
- **صفحة `/features` — أنيميشن فريد لكل ميزة** [`FeatureViz`](../apps/web/src/components/landing/FeatureViz.tsx): ١٢ ميزة، لكلٍّ رسم يعبّر عن وظيفتها (بلا تكرار): `crm` قمع صفقات · `quotes` عروض تتنافس وأفضلها يُبرَز · `policy` وثيقة يهبط عليها ختم + سطر ملحق · `ledger` قيد مزدوج مدين/دائن متوازن · `donut` حلقة نسبة تمتلئ · `claim` متتبّع حالة يتقدّم · `hr` شبكة حضور + قسيمة راتب · `aml` رادار يمسح + مؤشّر خطر · **`fingerprint` بصمة تُمسَح ضوئيًّا + علامة تحقّق** · `chart` مخطّط بخطّ اتجاه يُرسَم · `qr` رمز QR يُبنى + ختم موقّع · `tenants` حاويات معزولة حول درع. كلّها فوق لمعان دوريّ (`fx-sweep`) وبطاقات مرقّمة وارتفاع عند المرور.
